...
def import_bibtex(request):
    if request.method == "POST":
        ...
        if len(publications) == 0:
            ...
            msg = "Successfully added %i publications (%i skipped due to errors)" % (
                len(publications),
                len(errors),
            )
        else:
            msg = "Successfully added %i publication (%i error(s) occurred)" % (
                len(publications),
                len(errors),
            )
        ...
        return render(
            ...
            {"title": "Import BibTex", "types": Type.objects.all(), "request": request},
        )
